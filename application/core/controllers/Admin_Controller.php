...
class Admin_Controller extends MY_Controller {
	...
	// Get first letter from 2 words
	public function get_individual_code($full_name = '', $birth_date = '')
	{
		// Apparting all words from the variable
		$words = explode(' ', $full_name);
		$date_info = str_replace('-','',date('d-m-y',strtotime($birth_date)));
		...
		if (count($words)>1)
		{
			...
			{
				$letters .= $w[0];
				// Limiting only twice loop
				if ($i++ == 2) break;
			}

			$letters = $date_info . $letters;

		} else {
			
			$letters = $date_info . strtoupper(substr($full_name,0,2));
		}
		// var_dump($letters);
		return $letters;
	} 
